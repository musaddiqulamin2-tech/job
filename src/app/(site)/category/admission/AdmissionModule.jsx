...
export const ADMISSION_PAGE_DESCRIPTION =
...
function AdmissionHeader() {
  return (
    <div>
      ...
      >
        <p style={{ margin: 0, fontSize: "14px", color: "#52525b", lineHeight: 1.75 }}>{ADMISSION_PAGE_DESCRIPTION}</p>
      </div>
    </div>
  );
}

function AdmissionPagination({ current, total }) {
  return <Pagination currentPage={current} totalPages={total} basePath="/category/admission" />;
}

export default function AdmissionCategory({ posts, current, total }) {
  return (
              ...
              {posts.length > 0 ? (
                <>
                  <div className="ac-grid">
                    {posts.map((post, idx) => (
                      <AdmissionCard key={post._id} post={post} index={idx} />
                    ))}
                  </div>
                  <AdmissionPagination current={current} total={total} />
                </>
              ) : (
                <div className="ja-empty">
                  <p>No admission updates found.</p>
                </div>
              )}
            </div>
          </main>
